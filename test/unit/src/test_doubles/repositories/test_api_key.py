...
class TestFakeApiKeyStore:
    ...
    async def test_exists_by_hash_key_works(self):
        fake = FakeApiKeyStore()
    ...
    async def test_find_all_by_user_id_returns_active_sorted(self):
        fake = FakeApiKeyStore()
    ...
    async def test_count_by_user_id_filters_active(self):
        fake = FakeApiKeyStore()
        fake.set_fake_data([
    ...
    async def test_update_is_active_by_user_id_and_api_key_id_correctly(self):
        fake = FakeApiKeyStore()
        key = str(uuid.uuid4())
    ...
    async def test_update_is_active_handles_missing_or_bad_inputs(self):
        fake = FakeApiKeyStore()
        result = await fake.update_is_active_by_user_id_and_api_key_id("", None, False)
        assert result == -1
    
    async def test_count_by_user_id_empty_or_invalid(self):
        fake = FakeApiKeyStore()
...
@pytest.mark.asyncio
class TestStubApiKeyStore:
    async def test_output_mechanism(self) -> None:
        stub = StubApiKeyStore()

        save = stub.save
        find_all_by_user_id = stub.find_all_by_user_id
        ...
        expected_result = {
            ...
            find_all_by_user_id.__name__: generated_response,
        ...
        stub.set_output(find_all_by_user_id, expected_result.get(find_all_by_user_id.__name__) )
        ...
        await save(create_model=APIKeyRequestDTO(
            ...
            api_key=generated_response.api_key,
            masked_api_key=generated_response.masked_api_key,
            is_active=generated_response.is_active
        ))
        await find_all_by_user_id(offset=0, limit=10, user_id=generated_response.user_id)
