...
@pytest.mark.asyncio
class TestFakeQueueProcessingRegistryStore:
    async def test_save_inserts_and_sets_id_and_created_at(self):
    ...
    async def test_update_status_or_message_id_by_id_updates_status_only(self):
        store = FakeQueueProcessingRegistryStore()
    ...
    async def test_update_status_or_message_id_by_id_updates_both_when_message_id_given(self):
        store = FakeQueueProcessingRegistryStore()
        res = await store.save(
            QueueProcessingRegistryRequestDTO(
                message_id="m-1",
    ...
    async def test_update_status_or_message_id_by_id_invalid_inputs_return_minus_one(self):
        store = FakeQueueProcessingRegistryStore()
    ...
    async def test_update_status_or_message_id_by_id_id_not_found_returns_zero(self):
        store = FakeQueueProcessingRegistryStore()
        updated = await store.update_status_or_message_id_by_id(
    ...
    async def test_update_step_by_id_happy_path(self):
        store = FakeQueueProcessingRegistryStore()
        res = await store.save(
            QueueProcessingRegistryRequestDTO(
                message_id="m-1",
    ...
    async def test_update_step_by_id_invalid_inputs_return_minus_one(self):
        store = FakeQueueProcessingRegistryStore()
    ...
    async def test_update_step_by_id_id_not_found_returns_zero(self):
        store = FakeQueueProcessingRegistryStore()
        updated = await store.update_step_by_id(str(uuid.uuid4()), "done")
        assert updated == 0
    
    
    async def test_update_status_and_step_by_id_happy_path(self):
        store = FakeQueueProcessingRegistryStore()
        res = await store.save(
            QueueProcessingRegistryRequestDTO(
                message_id="m-1",
    ...
    async def test_update_status_and_step_by_id_invalid_inputs_return_minus_one(self):
        store = FakeQueueProcessingRegistryStore()
    ...
    async def test_update_status_and_step_by_id_id_not_found_returns_zero(self):
        store = FakeQueueProcessingRegistryStore()
        updated = await store.update_status_and_step_by_id(
    ...
    async def test_find_previous_latest_message_by_message_id_none_when_absent(self):
        store = FakeQueueProcessingRegistryStore()
