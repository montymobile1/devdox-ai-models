...
@pytest.mark.asyncio
async def test_set_fake_data_and_count():
    store = FakeRepoStore()
...
@pytest.mark.asyncio
async def test_save_inserts_and_sets_id():
    store = FakeRepoStore()
    req = RepoRequestDTO(
        user_id="u1",
        repo_id="rid-1",
...
@pytest.mark.asyncio
async def test_find_all_by_user_id_returns_multiple_within_limit():
    store = FakeRepoStore()
...
@pytest.mark.asyncio
async def test_count_by_user_id_counts_all():
    store = FakeRepoStore()
...
@pytest.mark.asyncio
async def test_get_by_id_with_string_uuid_succeeds():
    store = FakeRepoStore()
    a = make_repo_response(user_id="u1")
...
@pytest.mark.asyncio
async def test_find_by_repo_id_returns_match_or_none():
    store = FakeRepoStore()
...
@pytest.mark.asyncio
async def test_find_by_id_accepts_string_uuid_and_returns_item():
    store = FakeRepoStore()
    a = make_repo_response(user_id="u1")
    store.set_fake_data([a])
...
@pytest.mark.asyncio
async def test_find_by_user_id_and_html_url_first_match():
    store = FakeRepoStore()
...
@pytest.mark.asyncio
async def test_update_status_by_repo_id_updates_and_returns_codes():
    store = FakeRepoStore()
...
@pytest.mark.asyncio
async def test_save_context_creates_pending_and_id():
    store = FakeRepoStore()
...
@pytest.mark.asyncio
async def test_update_repo_system_reference_by_id_validations_and_update():
    store = FakeRepoStore()
    a = make_repo_response(user_id="u1")
    store.set_fake_data([a])
