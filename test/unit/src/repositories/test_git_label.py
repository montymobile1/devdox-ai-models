...
@pytest.mark.asyncio
async def test_save_adds_to_store():
...
@pytest.mark.asyncio
async def test_find_by_token_id_and_user_matches():
    store = FakeGitLabelStore()
...
@pytest.mark.asyncio
async def test_find_by_token_id_and_user_none():
    store = FakeGitLabelStore()
...
@pytest.mark.asyncio
async def test_find_git_hostings_by_ids_returns_matching():
    store = FakeGitLabelStore()
...
@pytest.mark.asyncio
async def test_find_all_by_user_id_respects_limit():
    store = FakeGitLabelStore()
...
@pytest.mark.asyncio
async def test_find_all_by_user_id_and_label_filters():
    store = FakeGitLabelStore()
...
@pytest.mark.asyncio
async def test_count_by_user_id_filters_git_hosting():
    store = FakeGitLabelStore()
...
@pytest.mark.asyncio
async def test_delete_by_id_and_user_id_removes_correct():
    store = FakeGitLabelStore()
    label = make_fake_git_label(user_id="u1")
    store.set_fake_data([label])
...
@pytest.mark.asyncio
async def test_find_by_id_and_user_id_and_git_hosting_matches():
    store = FakeGitLabelStore()
